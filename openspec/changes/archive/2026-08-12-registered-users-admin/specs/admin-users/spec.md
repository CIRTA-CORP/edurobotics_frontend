...
The system SHALL let an administrator view all registered users (students and
admins) with their name, username, email, role, registration date, and how many
courses each has started and completed.

#### Scenario: View users list
- **WHEN** an admin opens the "Usuarios" tab
- **THEN** the system fetches the list from `GET /api/admin/users`
- **AND THEN** each row shows name, email, role, courses started, courses completed and registration date
...
The system SHALL let an administrator promote another user to admin or return them
to student, with safeguards against lockout.
...
- **THEN** the system rejects it with a clear message and makes no change
